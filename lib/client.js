...
// used in xml creation of rpc message bodies
...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    if (input.parts) {
        ...
        message = new Js2Xml(alias + ":" + method.$name, arguments).toString()
            .replace(/^\<\?xml version="[\d\.]+" encoding="[\w\d\-]+"\?\>\s+/, '');
        (method.inputSoap === 'encoded') && (encoding = 'soap:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/" ');
    }
    else {
        // this assertion was breaking stuff, so for now its commented out
        //assert.ok(!style || style == 'document', 'invalid message definition for rpc style binding');
        message = self.wsdl.objectToDocumentXML(method.$name, arguments, input.targetNSAlias, input.targetNamespace);
    }
